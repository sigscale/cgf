...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% @doc Handle received HTTP HEAD requests.
%%%
%%% 	This is an {@link //inets/httpd. httpd} callback module handling
%%% 	HTTP HEAD operations. The HTTP resources are managed in modules named
%%% 	`cgf_rest_res_*'.
%%%
...
%%% 	{@link //cgf/cgf_rest:format_problem/2. format_problem/2} and the
...
parse_query(_, #mod{parsed_header = RequestHeaders,
	...
	{ContentType, ResponseBody}
			= cgf_rest:format_problem(Problem, RequestHeaders),
...
do_response(#mod{parsed_header = RequestHeaders,
	...
	Problem = #{type => "https://datatracker.ietf.org/doc/html/rfc7231#section-6.5.1",
			...
			code => "", status => 400},
	{ContentType, ResponseBody} = cgf_rest:format_problem(Problem, RequestHeaders),
	Size = integer_to_list(iolist_size(ResponseBody)),
	ResponseHeaders = [{content_length, Size}, {content_type, ContentType}],
...
do_response(#mod{parsed_header = RequestHeaders, data = Data} = ModData,
	...
	{ContentType, ResponseBody} = cgf_rest:format_problem(Problem1, RequestHeaders),
	Size = integer_to_list(iolist_size(ResponseBody)),
	ResponseHeaders = [{content_length, Size}, {content_type, ContentType}],
